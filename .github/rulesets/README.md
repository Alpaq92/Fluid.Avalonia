...
- required status check: **Build (Avalonia.Fluid)** (CI)
...
```bash
# create
gh api repos/Alpaq92/Avalonia.Fluid/rulesets --method POST --input .github/rulesets/main-branch-protection.json

# update later: find the id, then PUT
gh api repos/Alpaq92/Avalonia.Fluid/rulesets --jq '.[] | "\(.id)\t\(.name)"'
gh api repos/Alpaq92/Avalonia.Fluid/rulesets/<id> --method PUT --input .github/rulesets/main-branch-protection.json
```
