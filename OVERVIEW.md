...
- A Mica window. The custom title bar uses `SystemDecorations="BorderOnly"` so Avalonia's own drawn title/caption buttons are suppressed (no duplicated title) while the resize border is kept. The **Basic input** page also opens a plain Avalonia `Window` (a desktop-only modal example, blocked in the browser) so its OS title bar can be compared with this custom chrome.
